...
server <- function(input, output) {
    ...
    output$engagement <- renderPlotly({
        ...
        y <- user_filt %>% select(status_id, created_at) %>%
            mutate(time = as.POSIXct(ymd(as.Date(created_at)))) %>%
            group_by(time) %>%
            dplyr::summarise(nr_tweets = n())
        ...
        x <- x %>% filter(label %in% c("2_positive", "3_negative")) %>%
            ...
            inner_join(y, by="time")
        
        graph <- ggplot(x, aes(nr_reply, extreme)) + 
            geom_point(data=x,aes(size=nr_tweets, alpha=0.8)) + 
